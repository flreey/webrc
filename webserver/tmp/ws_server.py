...
class IrcThread(threading.Thread):
    def __init__(self, conn, lock, sself, senddata):
    ...
    def run(self):
        while self.active:
             infds,outfds,errfds = select.select([self.conn], [], [], self.timeout)
             #print '###%s' % infds
             if len(infds) != 0:
                buf = self.conn.recv(4096)
                print buf
                self.sself.senddata(buf)
                #self.senddata(self.sself, buf)
    def stop(self):
        self.active = False
...
class Broadcaster(Protocol):
    ...
    def dataReceived(self, msg):
        if msg.lower().find('upgrade: websocket') != -1:
            ...
            #     key.send_data('testhello')
            self.ircsend(raw_str)

    def ircsend(self, raw_str):
        self.conn.send(raw_str)
    ...
    def ircconnect(self):
        ...
        self.irct = IrcThread(self.conn, None, self, self.senddata)
        self.irct.start()
